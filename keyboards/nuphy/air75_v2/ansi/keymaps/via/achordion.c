...
bool process_achordion(uint16_t keycode, keyrecord_t* record) {
    ...
    // Allow user to bypass Achordion for specific keys/positions.
    if (IS_KEYEVENT(record->event) && achordion_exclude(keycode, record)) {
        return true;
    }
    ...
    if (achordion_state == STATE_UNSETTLED && record->event.pressed) {
        ...
        streak_active = streak_timer && s_timeout &&
        ...
        // tap-hold key as tapped vs. held.
        //
        // We implement the tap or hold by plumbing events back into the handling
        // pipeline so that QMK features and other user code can see them. This is
        // done by calling `process_record()`, which in turn calls most handlers
        // including `process_record_user()`.
        const bool chord = !streak_active && (!is_key_event ||
...
#ifdef ACHORDION_STREAK
            update_streak_timer(keycode, record);
            if (streak_active && is_key_event && is_tap_hold && record->tap.count == 0) {
...
__attribute__((weak)) bool achordion_exclude(uint16_t keycode, keyrecord_t* record) {
    (void)keycode;
    (void)record;
    return false;
}
...
__attribute__((weak)) uint16_t achordion_streak_chord_timeout(uint16_t tap_hold_keycode,
